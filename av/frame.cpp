...
static AVFrame* allocate()
{
  ...
  if (!frame) {
    throw Common::Exception("Failed to allocate AVFrame");
  }
  return frame;
}
  ...
  : frame_(allocate())
{
  ...
  if (ret < 0) {
    throw Common::Exception("Failed to allocate memory for frame data");
  }
}
...
{
  if (!frame_) {
    throw Common::Exception("Failed to copy AVFrame");
  }
}
